...
logger = logging.getLogger("scripts.events")
...
class Events:
    ...
    def ceremony(self, cat, promoted_to, preparedness="prepared"):
        ...
        try:
            ...
            logger.exception(
                "Issue gathering ceremony text for %s (promoting to %s)",
                str(cat.name),
                promoted_to,
                exc_info=ex,
            )
    ...
    def handle_murder(self, cat):
        ...
        if targets:
            ...
            if not int(random.random() * kill_chance):
                logger.info(
                    "Murder triggered - %s targeting %s",
                    cat.name,
                    Cat.fetch_cat(chosen_target.cat_to).name,
                )
