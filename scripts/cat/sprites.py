...
class Sprites:
    ...
    def load_tints(self):
        try:
            ...
            logger.exception("Could not read tints")

        try:
            with open("sprites/dicts/white_patches_tint.json", "r") as read_file:
                self.white_patches_tints = ujson.loads(read_file.read())
        except IOError:
            logger.exception("Could not read white patches tints")
        ...
        for y in range(sprites_y):
            for x in range(sprites_x):
                ...
                try:
                    ...
                    logger.warning(f"Sprite %s does not exist", full_name)
    ...
    def load_all(self):
        ...
        if isinstance(self.size, int):
            ...
            logger.warning(f"lineart.png is not 3x7, falling back to %s", self.size)
            logger.info(
